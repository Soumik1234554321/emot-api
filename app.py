...
# LAG TEAM FUNCTION - Fast join/leave loop with 1 minute auto-stop (CHANGED FROM 5 MINUTES)
async def lag_team_loop(team_code, key, iv, region):
    """Rapid join/leave loop to create lag with 1 minute auto-stop"""
    global lag_running, online_writer, lag_start_time
    count = 0
    lag_start_time = time.time()  # Record start time
    max_duration = 1 * 60  # 1 minute in seconds (CHANGED FROM 5 MINUTES)
    
    print(f"🚀 Lag attack started for team: {team_code}")
    print(f"⏰ Auto-stop in: 1 minute")  # CHANGED FROM 5 MINUTES
    ...
    while lag_running:
        try:
            ...
            if elapsed_time >= max_duration:
                print("⏰ 1 minute completed! Auto-stopping lag attack...")  # CHANGED FROM 5 MINUTES
                lag_running = False
                
                # Send notification in chat
                notification_msg = f"[B][C][FF0000]⏰ LAG AUTO-STOPPED!\nTime: 1 minute completed\nTeam: {team_code}\n"  # CHANGED
                try:
            ...
            if count % 1000 == 0:  # Every 10 seconds (1000 cycles at 0.01s each)
                print(f"⏳ Lag in progress... {minutes_left}m {seconds_left}s remaining")
    ...
    print(f"✅ Lag attack stopped. Total cycles: {count}")
            ...
            while True:
                ...
                if data.hex().startswith("120000"):
                    ...
                    if response:
                        ...
                        # NEW LAG COMMAND - UPDATED TO 1 MINUTE
                        if inPuTMsG.strip().startswith('/lag '):
                            ...
                            if len(parts) < 2:
                                ...
                                # SUCCESS MESSAGE - UPDATED TO 1 MINUTE
                                success_msg = f"[B][C][00FF00]✅ LAG ATTACK STARTED!\nTeam: {team_code}\nDuration: 1 minute (auto-stop)\nSpeed: Ultra fast (10ms)\n"
                        ...
                        if inPuTMsG.strip() == '/stop lag':
                            if lag_task and not lag_task.done():
                                lag_running = False
                                lag_task.cancel()
                                success_msg = f"[B][C][00FF00]✅ LAG ATTACK STOPPED!\n"
                                P = await SEndMsG(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                            else:
                        ...
                        # LAG STATUS COMMAND - UPDATED TO 1 MINUTE
                        if inPuTMsG.strip() == '/lag status':
                            if lag_task and not lag_task.done() and lag_start_time:
                                current_time = time.time()
                                elapsed_time = current_time - lag_start_time
                                remaining_time = (1 * 60) - elapsed_time  # CHANGED TO 1 MINUTE
                                
                                if remaining_time > 0:
                                    minutes_left = int(remaining_time // 60)
                                    seconds_left = int(remaining_time % 60)
                                    status_msg = f"[B][C][FFFF00]📊 LAG STATUS\nActive: ✅ YES\nTime left: {minutes_left}m {seconds_left}s\nAuto-stop: 1 minute total\n"  # CHANGED
                                else:
                                    status_msg = f"[B][C][00FF00]📊 LAG STATUS\nActive: ✅ FINISHED\nStatus: Auto-stopped (1 minute completed)\n"  # CHANGED
                            else:
                                status_msg = f"[B][C][FF0000]📊 LAG STATUS\nActive: ❌ NO\nNo lag attack running\n"
...
# NEW API ENDPOINT FOR LAG TEAM with 1 minute auto-stop (CHANGED FROM 5 MINUTES)
@app.route('/lag')
def lag_team_api():
    ...
    # Start new lag task with 1 minute auto-stop
    lag_running = True
    ...
    return jsonify({
        ...
        "team_code": team_code,
        "duration": "1 minute (auto-stop)",  # CHANGED FROM 5 MINUTES
        "action": "Rapid join/leave loop",
        "speed": "Ultra fast (10ms intervals)",
        "auto_stop": "Yes, after 1 minute"  # CHANGED FROM 5 MINUTES
    })
...
# API endpoint to check lag status - UPDATED FOR 1 MINUTE
...
def lag_status_api():
    ...
    if lag_task and not lag_task.done() and lag_start_time:
        current_time = time.time()
        elapsed_time = current_time - lag_start_time
        total_duration = 1 * 60  # 1 minute in seconds (CHANGED FROM 5 MINUTES)
        ...
        if remaining_time > 0:
            ...
            return jsonify({
                "status": "finished",
                "message": "Lag attack auto-stopped (1 minute completed)",  # CHANGED
                "duration_completed": "1 minute"  # CHANGED
            })
    else:
        return jsonify({
            "status": "inactive",
